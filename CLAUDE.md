...
| `docker-compose.yml` | Local dev stack: Postgres, RabbitMQ, Temporal (+ UI), Grafana/Loki/Tempo/Prometheus, backend, plus `docs` via `include: [docs/compose.yaml]`. Credentials come from a gitignored `.env` — never hardcode them into a compose file itself; see `.env.example`. |
